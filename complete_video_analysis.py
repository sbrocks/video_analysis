...
while cam.isOpened():
    ...
    if ret==True:
        ...
        for (i, rect) in enumerate(rects):
            ...
            detected_face = img[int(ty):int(ty+th), int(tx):int(tx+tw)]
...
angry_percentage = float(emotion_angry/num_frames)*100
sad_percentage = float(emotion_sad/num_frames)*100
happy_percentage = float(emotion_happy/num_frames)*100
surprise_percentage = float(emotion_surprise/num_frames)*100
disgust_percentage = float(emotion_disgust/num_frames)*100
fear_percentage = float(emotion_fear/num_frames)*100
neutral_percentage = float(emotion_neutral/num_frames)*100  
